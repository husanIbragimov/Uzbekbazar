...
from app.products.models import Size, Colors
...
class ShopCartAddOneView(LoginRequiredMixin, View):
    def get(self, request, uuid):
        ...
        if ShopCart.objects.filter(user=user, product=product).exists():
            ...
            product_price = \
            product.product_size.filter(Q(color=product_color[0]) & Q(size=product_size[0]) & Q(is_active=True))[
                0].price
    ...
    def post(self, request, uuid):
        ...
        product_price = \
        product.product_size.filter(Q(color=product_color[0]) & Q(size=product_size[0]) & Q(is_active=True))[0].price
        if product.percentage:
            product_price = product_price - (product_price * product.percentage) / 100
    ...
    def get(self, request):
        ...
        if shop_cart:
            for shop_cat in shop_cart:
                if shop_cat.product.product_size.get(
                        Q(color=shop_cat.product_color) & Q(size=shop_cat.product_size) & Q(
                    ...
                    for shop_cart_item in shop_cart:
                        data.append(OrderItem(order=order, product=shop_cart_item.product, quenty=shop_cart_item.quenty,
                                              product_image=shop_cart_item.product_image,
                                              product_size=shop_cart_item.product_size,
                                              product_color=shop_cart_item.product_color,
                                              product_price=shop_cart_item.product_price))
...
class OrderVariantView(LoginRequiredMixin, View):
    ...
    def post(self, request, uuid):
        ...
        if int(current_variant) == 0:
            ...
            order = OrderVariant.objects.create(product=product, user=request.user,
                                                color=Colors.objects.get(uuid=current_color),
                                                size=Size.objects.get(uuid=current_size),
                                                variant=Variant.objects.get(id=current_variant), product_image=
                                                product.product_images.filter(
                                                    Q(color=Colors.objects.get(uuid=current_color)))[0])
            context = {
                'order': order
            }
